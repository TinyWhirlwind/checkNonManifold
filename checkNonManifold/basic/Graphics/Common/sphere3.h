...
template <class T> void Sphere3<T>::Intersect(const Sphere3<T> &s) {
 ...
 template <class T> int Sphere3<T>::CreateFromBox(int n,  const Point3<T> *points) {
   Point3f max1(-1e100, -1e100, -1e100);
   Point3f min1(1e100, 1e100, 1e100);
   for(int i = 0; i < n; i++) {
     for(int k = 0; k < 3; k++) {
       if(max1[k] < points[i][k]) max1[k] = points[i][k];
       if(min1[k] > points[i][k]) min1[k] = points[i][k];
     }
   }
   Center() = (min1 + max1)/2;
   Radius() = (min1 - max1).Norm()/2;
   return 0;
 }
