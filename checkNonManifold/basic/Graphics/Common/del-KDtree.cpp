...
KDtree* CreateKDtree (MeshData * mesh,int nPts)
{
  ...
  KDtree* kdtree = new KDtree(mesh,inds,mesh->vn);

  delete[] inds;

  return kdtree;
}
static int divisionsorts(MeshData * mesh, int *p, int n,int dim, float med)
{
...
KDtree::KDtree(MeshData * mesh,int *ind,int n, int first)
...
#undef DATA
    ...
    // recurse
    child[0] = new KDtree(mesh, ind,right, 0);
    child[1] = new KDtree(mesh, &ind[right],n-right, 0);
  } 
  else
  {
    // store data here
...
bool KDtree::_search(MeshData * mesh,const Point3f &p,int num,std::vector<int> & indices,std::vector<float> & d2) const
{
  assert(this);
