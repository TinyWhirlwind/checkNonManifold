...
/*
 ...
 V(0) : scalar part of quaternion
 V(1) - V(3): vector part of quaternion
...
namespace sn3DCore
{
	...
	template <class S> void Quaternion<S>::FromAxis(const S phi, const Point3<S> &a)
	{
		Point3<S> b = a;
		b.Normalize();
		S s = AXYMath::Sin(phi / (S(2.0)));

		V(0) = AXYMath::Cos(phi / (S(2.0)));
		V(1) = b[0] * s;
		V(2) = b[1] * s;
		V(3) = b[2] * s;
	}
	template <class S> void Quaternion<S>::ToAxis(S &phi, Point3<S> &a) const {
		S s = AXYMath::Asin(V(0))*S(2.0);
		phi = AXYMath::Acos(V(0))*S(2.0);
	...
	template <class S> void Quaternion<S>::ToMatrixGL(S mat[]) const
	{
		S q00 = V(1)*V(1);
		S q01 = V(1)*V(2);
		S q02 = V(1)*V(3);
		...
		mat[0] = (S)(1.0 - (q11 + q22)*2.0);
		mat[4] = (S)((q01 - q23)*2.0);
		mat[8] = (S)((q02 + q13)*2.0);
		mat[12] = (S)0.0;

		mat[1] = (S)((q01 + q23)*2.0);
		mat[5] = (S)(1.0 - (q22 + q00)*2.0);
		mat[9] = (S)((q12 - q03)*2.0);
		mat[13] = (S)0.0;

		mat[2] = (S)((q02 - q13)*2.0);
		mat[6] = (S)((q12 + q03)*2.0);
		mat[10] = (S)(1.0 - (q11 + q00)*2.0);
		mat[14] = (S)0.0;
	...
	template <class S> void Quaternion<S>::FromMatrix(Matrix44<S> &m) {
		...
		if (t > 0) {
			...
			if (m.V()[0] > m.V()[5] && m.V()[0] > m.V()[10]) {
				Sc = AXYMath::Sqrt((S)1.0 + m.V()[0] - m.V()[5] - m.V()[10]) * (S)2.0;
				V(1) = (S)0.5 / Sc;
				V(2) = (m.V()[1] + m.V()[4]) / Sc;
				V(3) = (m.V()[2] + m.V()[8]) / Sc;
				V(0) = (m.V()[6] + m.V()[9]) / Sc;
				...
				Sc = AXYMath::Sqrt((S)1.0 + m.V()[5] - m.V()[0] - m.V()[10]) * (S)2.0;
				V(1) = (m.V()[1] + m.V()[4]) / Sc;
				V(2) = (S)0.5 / Sc;
				V(3) = (m.V()[6] + m.V()[9]) / Sc;
				V(0) = (m.V()[2] + m.V()[8]) / Sc;
				...
				Sc = AXYMath::Sqrt((S)1.0 + m.V()[10] - m.V()[0] - m.V()[5]) * (S)2.0;
				V(1) = (m.V()[2] + m.V()[8]) / Sc;
				V(2) = (m.V()[6] + m.V()[9]) / Sc;
				V(3) = (S)0.5 / Sc;
				V(0) = (m.V()[1] + m.V()[4]) / Sc;
	...
	{
		//...TODO...
		alpha = atan(2 * (V(0)*V(1) + V(2)*V(3)) / (float)(1 - 2 * V(1)*V(1) - 2 * V(2)*V(2)));
		beta = asin(2 * (V(0)*V(2) - V(1)*V(3)));
		gamma = atan(2 * (V(0)*V(3) + V(1)*V(2)) / (float)(1 - 2 * V(3)*V(3) - 2 * V(2)*V(2)));
	}
	...
	template <class S> Quaternion<S> Interpolate(Quaternion<S>   a, Quaternion<S>   b, double t) {
		...
		double phi = AXYMath::Acos(v);
		if (phi > 0.01) {
			a = a  * (AXYMath::Sin(phi *(1 - t)) / AXYMath::Sin(phi));
			b = b  * (AXYMath::Sin(phi * t) / AXYMath::Sin(phi));
		}
	...
	template <class S> void  QuaternionMultiply(Quaternion<S> & q, Quaternion<S> q1, Quaternion<S> q2)
	{
	...
	template <class S> void  GetMatrixFromQuaternion(Quaternion<S> q, float * matrix)
	{
