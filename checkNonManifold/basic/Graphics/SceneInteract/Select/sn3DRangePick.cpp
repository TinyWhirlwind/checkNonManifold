...
namespace sn3DCore
{
	sn3DRangePick::sn3DRangePick(sn3DRangeModel * model)
	{
		std::vector<Point3f> &pnts =model->GetRange()->GetPoints();
		if(pnts.size()>3 )
		{
			m_SpaceBox.Initialize(&pnts[0],pnts.size(),50);
		}
	}
	void sn3DRangePick::PaintVertex(sn3DObject * mesh,Quaternionf rot,int flag)
	{

		sn3DRangeModel * model = (sn3DRangeModel*)mesh;
		sn3DRangeData * data = model->GetRange();
				...
				for (int w=0;w<scale;w++)
				{
					...
					else if(inspace[0]||inspace[1]||inspace[2]||inspace[3]||inspace[4]||inspace[5]||inspace[6]||inspace[7])
					{
						...
						for(int j=0;j<size;j++)
						{
							...
							if(model->IsS(item))continue;
							if(model->IsD(item))continue;

							Point3f pp = data->GetPoints()[item];
							//Point3f pp = data->m_pnts[item];
	...
	void sn3DRangePick::SelectPolygonVertex(sn3DObject * m,Quaternionf rot,int flag)
	{
		sn3DRangeModel * model = (sn3DRangeModel*)m;
		sn3DRangeData * data = model->GetRange();
				...
				for (int w=0;w<scale;w++)
				{
					...
					if(inspace[0]||inspace[1]||inspace[2]||inspace[3]||inspace[4]||inspace[5]||inspace[6]||inspace[7])
					{
						for(int j=0;j<size;j++)
						{
							...
							point.X() = data->GetPoints()[item].X();
							point.Y() = data->GetPoints()[item].Y();
							point.Z() = data->GetPoints()[item].Z();
							point.W() = 1.0;
							...
							if(CheckInArea(p,rpoints))model->SetS(item);			
...
void sn3DRangePick::SelectRefPolygonVertex(sn3DObject * m,Quaternionf rot,int flag)
	{
		sn3DRangeModel * model = (sn3DRangeModel*)m;
		sn3DRangeData * data = model->GetRange();
		...
		int refpointnum = model->m_refNum;
		model->m_refSelectFlag.resize(refpointnum);
		for (int i=0; i<refpointnum; i++)
		{
			if (model->IsRefpointSelect(i)) continue;

			Quaternionf  point;
			point.X() =  model->m_refPts[i].X();
			point.Y() =  model->m_refPts[i].Y();
			point.Z() =  model->m_refPts[i].Z();
			point.W() = 1.0;
			...
			p.Z() = result.Z();
			//Point3f p;
		    //p.X() = model->m_refPts[i].X();
			//p.Y() = model->m_refPts[i].Y();
			//p.Z() = model->m_refPts[i].Z();

            //model->ClearRefpointSelect(i);
			if(CheckInArea(p,rpoints))
			{
				model->SetRefpointSelect(i);
	...
	void sn3DRangePick::RectVertex(sn3DObject * m,Quaternionf rot,int flag)
	{
	...
	bool sn3DRangePick::CheckInArea(Point3f point,std::vector<Point3f> &  line)
	{
