...
public: 
  ...
  Pnt3(const double *a)   { v[0] = (float)(a[0]), v[1] = (float)(a[1]), v[2] = (float)(a[2]);}
  ...
  Pnt3 &set(const double *a)
    { v[0] = (float)a[0], v[1] = (float)a[1], v[2] = (float)a[2]; return *this; }
	  ...
	  const Pnt3 &t1, const Pnt3 &t2, const Pnt3 &t3)
{
  ...
  Pnt3 n = cross(v1,v2);
  float n_inv_mag2 = (float)(1.0/n.norm2());
     ...
     float &b1, float &b2, float &b3)
{
  ...
  if (fabs(v10*v21 - v11*v20) > c[i]) {
    // ignore z
    float d = (float)(1.0 / (v10*v21-v11*v20));
    ...
    // ignore x
    float d = (float)(1.0 / (v11*v22-v12*v21));
    ...
    // ignore y
    float d = (float)(1.0 / (v12*v20-v10*v22));
	  ...
	  float &b1, float &b2, float &b3)
{
  ...
  if (n.v[2] > n.v[i]) {
    // ignore z
    float d = (float)(1.0 / (v1.v[0]*v2.v[1]-v1.v[1]*v2.v[0]));
    ...
    // ignore x
    float d = (float)(1.0 / (v1.v[1]*v2.v[2]-v1.v[2]*v2.v[1]));
    ...
    // ignore y
    float d = (float)(1.0 / (v1.v[2]*v2.v[0]-v1.v[0]*v2.v[2]));
...
Pnt3::smallest_circle(const Pnt3 &A, const Pnt3 &B, const Pnt3 &C)
{
  ...
  if (da > db && da > dc) {
    ...
    if (da >= db + dc) {
      ...
      // longest edge
      v[0] = (float)(.5 * (A.v[0]+B.v[0]));
      v[1] = (float)(.5 * (A.v[1]+B.v[1]));
      v[2] = (float)(.5 * (A.v[2]+B.v[2]));
      return (float)(.5*sqrtf(da));
    }
  } else if (dc > db) {
    // dc longest
    if (dc >= db + da) {
      // over 90 deg angle, solution is the center of the 
      // longest edge
      v[0] = (float)(.5 * (A.v[0]+C.v[0]));
      v[1] = (float)(.5 * (A.v[1]+C.v[1]));
      v[2] = (float)(.5 * (A.v[2]+C.v[2]));
      return (float)(.5*sqrtf(dc));
    }
  } else {
    // db longest
    if (db >= da + dc) {
      // over 90 deg angle, solution is the center of the 
      // longest edge
      v[0] = (float)(.5 * (B.v[0]+C.v[0]));
      v[1] = (float)(.5 * (B.v[1]+C.v[1]));
      v[2] = (float)(.5 * (B.v[2]+C.v[2]));
      return (float)(.5*sqrtf(db));
    }
  }
  ...
  v[0] =(float)( A.v[0] + .5 * (a.v[0] + fact * aperp.v[0]));
  v[1] = (float)(A.v[1] + .5 * (a.v[1] + fact * aperp.v[1]));
  v[2] = (float)(A.v[2] + .5 * (a.v[2] + fact * aperp.v[2]));
  return (float)(.5 * sqrtf(da*fact*fact+da));
}
...
{
  float x = (float)(m[0]*v[0] + m[4]*v[1] + m[8] *v[2] + m[12]);
  float y = (float)(m[1]*v[0] + m[5]*v[1] + m[9] *v[2] + m[13]);
  v[2]    = (float)(m[2]*v[0] + m[6]*v[1] + m[10]*v[2] + m[14]);
  v[0] = x; v[1] = y;
  return *this;
}
...
Pnt3::xform(float r[3][3], double t[3])
{
  float x = (float)(t[0] + r[0][0]*v[0] + r[0][1]*v[1] + r[0][2]*v[2]);
  float y = (float)(t[1] + r[1][0]*v[0] + r[1][1]*v[1] + r[1][2]*v[2]);
  v[2]    = (float)(t[2] + r[2][0]*v[0] + r[2][1]*v[1] + r[2][2]*v[2]);
  v[0] = x; v[1] = y;
  return *this;
}
...
Pnt3::invxform(double m[16])
{
  ...
  double ty = v[1] - m[13];
  double tz = v[2] - m[14];
  v[0] = (float)(m[0]*tx + m[1]*ty + m[2]*tz);
  v[1] = (float)(m[4]*tx + m[5]*ty + m[6]*tz);
  v[2] = (float)(m[8]*tx + m[9]*ty + m[10]*tz);
  return *this;
}
...
Pnt3::invxform(float r[3][3], double t[3])
{
  float tx = (float)(v[0] - t[0]);
  float ty = (float)(v[1] - t[1]);
  float tz = (float)(v[2] - t[2]);
  v[0] = r[0][0]*tx + r[1][0]*ty + r[2][0]*tz;
  v[1] = r[0][1]*tx + r[1][1]*ty + r[2][1]*tz;
  v[2] = r[0][2]*tx + r[1][2]*ty + r[2][2]*tz;
  return *this;
