...
{
  m[0][0] = (T)a[0]; m[0][1] = (T)a[1]; m[0][2] = (T)a[2]; m[0][3] = (T)a[3];
  m[1][0] = (T)a[4]; m[1][1] = (T)a[5]; m[1][2] = (T)a[6]; m[1][3] = (T)a[7];
  m[2][0] = (T)a[8]; m[2][1] = (T)a[9]; m[2][2] = (T)a[10];m[2][3] = (T)a[11];
  m[3][0] = (T)a[12];m[3][1] = (T)a[13];m[3][2] = (T)a[14];m[3][3] = (T)a[15];
}
			 ...
			 double tx, double ty, double tz)
{
  ...
  T norm2 = (T)(q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3]);
  T s = (T)((norm2 > 0.0) ? 2.0/norm2 : 0.0);
  T xs = (T)(q[1]*s),   ys = (T)(q[2]*s),   zs = (T)(q[3]*s);
  T wx = (T)(q[0]*xs),  wy = (T)(q[0]*ys),  wz = (T)(q[0]*zs);
  T xx = (T)(q[1]*xs),  xy = (T)(q[1]*ys),  xz = (T)(q[1]*zs);
  T yy = (T)(q[2]*ys),  yz = (T)(q[2]*zs),  zz = (T)(q[3]*zs);
  
  m[0][0] = (T)(1.0 - (yy+zz));  m[0][1] =(T)(xy + wz);  m[0][2] =(T)( xz - wy);
  m[1][0] = (T)(xy - wz);  m[1][1] = (T)(1.0 - (xx+zz));  m[1][2] = (T)(yz + wx);
  m[2][0] = (T)(xz + wy);  m[2][1] = (T)(yz - wx);  m[2][2] = (T)(1.0 - (xx+yy));
  
  m[3][0] = (T)tx;  m[3][1] = (T)ty;  m[3][2] = (T)tz;
  m[0][3] = m[1][3] = m[2][3] = (T)0.0;
  m[3][3] = (T)1.0;
...
{
  rotQ((T)q[0],(T)q[1],(T)q[2],(T)q[3]);
  if (n == 7)
    translate((T)q[4],(T)q[5],(T)q[6]);
  return *this;
}
...
{
  T invw = 
    (T)(1.0 / (m[0][3]*i[0]+m[1][3]*i[1]+m[2][3]*i[2]+m[3][3]));
  o[0] = (m[0][0]*i[0]+m[1][0]*i[1]+m[2][0]*i[2]+m[3][0])*invw;
  o[1] = (m[0][1]*i[0]+m[1][1]*i[1]+m[2][1]*i[2]+m[3][1])*invw;
  o[2] = (m[0][2]*i[0]+m[1][2]*i[1]+m[2][2]*i[2]+m[3][2])*invw;
}
...
Xform<T>::operator()(float i[3]) const
{
  ...
  T invw = (T)(
    1.0 / (m[0][3]*o[0]+m[1][3]*o[1]+m[2][3]*o[2]+m[3][3]));
  i[0] = (float)((m[0][0]*o[0]+m[1][0]*o[1]+m[2][0]*o[2]+m[3][0])*invw);
  i[1] = (float)((m[0][1]*o[0]+m[1][1]*o[1]+m[2][1]*o[2]+m[3][1])*invw);
  i[2] = (float)((m[0][2]*o[0]+m[1][2]*o[1]+m[2][2]*o[2]+m[3][2])*invw);
}
...
Xform<T>::apply_nrm(const float i[3], float o[3]) const // o = M*i
{
  double invw = 
    1.0 / (m[0][3]*i[0]+m[1][3]*i[1]+m[2][3]*i[2]+m[3][3]);
  o[0] = (float)((m[0][0]*i[0]+m[1][0]*i[1]+m[2][0]*i[2])*invw);
  o[1] = (float)((m[0][1]*i[0]+m[1][1]*i[1]+m[2][1]*i[2])*invw);
  o[2] = (float)((m[0][2]*i[0]+m[1][2]*i[1]+m[2][2]*i[2])*invw);
}
...
/*
template <class T> inline Xform<T> 
operator*(const Xform<T> &a,const Xform<T> &b) // (*this) = a * (*this)
{
	/*Xform<T> b((T*)this);
	for (int i=0; i<4; i++) {
		for (int j=0; j<4; j++) {
...
template <class T> Xform<T>
multipleXF(const Xform<T> &a, const Xform<T> &b)
{ 
  Xform<T> out;
  for (int i=0; i<4; i++) {
    for (int j=0; j<4; j++) {
